...
class Entrypoint extends AutomationTestSetup {
    ...
    constructor() {
        super();
        this.headless = process.env.HEADLESS ? JSON.parse(process.env.HEADLESS) : false;
    ...
    async addTestSelection(page: Page, browser: Browser) {
        ...
        await page.evaluate((tests) => {
            ...
            select.style.cssText =
                'position:fixed; top: 15px; left:15px;  z-index: 10000000000000000000000000000000; width: auto; height: auto; font-family: serif; cursor: pointer; outline: none; opacity:.3; transition: opacity .2s; font-size: 18px;';
    ...
    async addTestRunner(page: Page, browser: Browser) {
        ...
        await page.evaluate(
            async (headless, tests) => {
                ...
                if (headless) {
                    testSelected(tests[tests.length - 1].name);
                }
            },
            this.headless,
            tests,
