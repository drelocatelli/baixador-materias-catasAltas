...
class ExampleSpecCopy extends AutomationTestSetup {
    pageNumber: number = 0;
    page: Page;
    browser: Browser;
    ...
    async nextPage() {
        ...
        await this.page.waitForTimeout(process.env.TIMEOUT ? parseInt(process.env.TIMEOUT) : 1000);
    ...
    async downloadPosts(page: Page): Promise<void> {
        ...
        posts = await listGroup?.evaluate((el) => {
            const listGroupItems: NodeListOf<HTMLAnchorElement> = document.querySelectorAll('#ultimas_ul.list-group > .list-group-item');
            let items: string[] = [];
        ...
        for await (let post of posts) {
            ...
            await pageMateria.waitForTimeout(process.env.TIMEOUT ? parseInt(process.env.TIMEOUT) : 800);
            ...
            const postContent = await entryContent?.evaluate((data) => {
                ...
                postContent.date = date.innerText;
...
export default ExampleSpecCopy;
